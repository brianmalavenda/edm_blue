=== Newsical ===
Contributors: afthemes
Requires PHP: 5.3
Requires at least: 4.0
Tested up to: 6.7
Stable tag: 1.0.15
License: GNU General Public License v2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Tags: news, blog, entertainment, one-column, two-columns, three-columns, four-columns, grid-layout, block-patterns, block-styles, left-sidebar, right-sidebar, custom-header, flexible-header, custom-background, custom-logo, custom-menu, custom-colors, featured-images, full-width-template, post-formats, rtl-language-support, footer-widgets, translation-ready, theme-options, threaded-comments, wide-blocks

Newsical is a versatile WordPress theme tailored for blogs, magazines, and news sites. The 1-click demo import and ready-made designs make setup simple and fast WITHOUT CODING. It’s compatible with popular page builders like Gutenberg, Elementor, Beaver Builder, Divi, Brizy, and more, offering flexible design options. Fully responsive and SEO-friendly, Newsical supports key plugins like WooCommerce, Jetpack, Contact Form 7, and Yoast SEO for creating feature-rich websites. With multilingual and RTL support—including demo content in languages such as English, Spanish, French, Japanese, and Arabic—it’s perfect for global audiences. Lightweight and optimized for performance, Newsical ensures faster load times and better SEO results. Its professional layouts and customizable sections, widgets, template kits & block patterns help you highlight your content effectively. Learn more and explore demos at afthemes.com/products/newsical/

== Copyright ==

This theme, like WordPress, is licensed under the GPL.
Use it to make something cool, have fun, and share what you've learned with others.

Newsical is based on Underscores https://underscores.me/, (C) 2012-2017 Automattic, Inc.
Underscores is distributed under the terms of the GNU GPL v2 or later.

Newsical is a child theme of "MoreNews". MoreNews Theme is licensed under the GPL v2. Copyright 2024 AF themes
Newsical is distributed under the terms of the GNU GPL v2 or later.

== Description ==
 Newsical is a sleek, high-performance WordPress theme designed for blog, magazine, and news publishers who want a striking dark theme. Offering full compatibility with major page builders like Elementor, Beaver Builder, Brizy, Visual Composer, Divi, and SiteOrigin, Newsical provides unmatched design flexibility. It’s fully responsive, SEO-optimized, and supports popular plugins such as WooCommerce, Jetpack, Contact Form 7, and Yoast SEO, allowing you to build a complete, dynamic site. With multilingual and RTL support—including demo content in languages like English, Chinese, Spanish, French, Japanese, and Arabic—Newsical is ideal for global publishers. The theme’s 1-click demo import feature and pre-built demos make setup quick and intuitive. Designed to be lightweight and fast-loading, it ensures optimal SEO and performance for increased engagement and reach. Perfect for blogs, magazines, and news sites, Newsical offers a professional, adaptable dark design with a wide array of layout and block patterns to showcase your content effectively. Explore demos and documentation at https://afthemes.com/products/newsical/

== Installation ==

1. In your WordPress dashboard, go to Appearance > Themes and click Add New.
2. Click on Upload Theme, then select the theme’s .zip file from your computer. Click Install Now.
3. After installation, click Activate to start using the theme.
4. To import demo content, go to Starter Sites under the Newsical menu in your dashboard.
5. To import Gutenberg block patterns, go to Block Patterns under the Newsical menu.
6. To import Elementor template kits, go to Elementor Kits under the Newsical menu.

== Frequently Asked Questions ==

Newsical includes support for the following plugins:
- Gutenberg: Supports a variety of block patterns for enhanced customization.
- Elementor: Compatible with template kits and custom widgets for advanced layouts.
- Beaver Builder: Compatible with templates and custom sections for advanced layouts.
- Brizy: Compatible with templates and custom sections for advanced layouts.
- WooCommerce: Fully compatible with eCommerce functionality, including product grids and custom shop layouts.
- Yoast SEO: Supports enhanced SEO features and breadcrumbs for improved visibility.
- Contact Form 7: Easily create contact forms and integrate them into your pages.
- Jetpack: Compatible with Infinite Scroll, Content Options, and Related Posts.
- WPML: Ready for multilingual websites, making translation and localization seamless.
- gTranslate: Ready for multilingual websites, making translation and localization seamless.
- WeGlot: Ready for multilingual websites, making translation and localization seamless.


== Change-log ==
= 1.0.15 - Jan 14 2025 =
* Style update

= 1.0.14 - Jan 10 2025 =
* Style update

= 1.0.13 - Jan 08 2025 =
* Style update

= 1.0.12 - Jan 04 2025 =
* Style update

= 1.0.11 - Jan 02 2025 =
* Style update

= 1.0.10 - Dec 31 2024 =
* Style update

= 1.0.9 - Dec 30 2024 =
* Style update

= 1.0.8 - Dec 29 2024 =
* Style update

= 1.0.7 - Dec 28 2024 =
* Style update

= 1.0.6 - Dec 27 2024 =
* Style update

= 1.0.5 - Dec 26 2024 =
* Style update

= 1.0.4 - Dec 23 2024 =
* Style update

= 1.0.3 - Dec 20 2024 =
* Style update

= 1.0.2 - Dec 13 2024 =
* Style update

= 1.0.1 - Dec 07 2024 =
* Info update

= 1.0.0 - Dec 02 2024 =
* Initial release


------------------------------------------------------------------------------
Image License
------------------------------------------------------------------------------
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/99818

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1081782

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1047693

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/2767

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/658894

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1324628